...
pip install "lazyportfolio @ git+https://github.com/selvaz/LazyPortfolio.git"  # or straight from GitHub
lazyportfolio-setup
```

`lazyportfolio-setup` ships *inside* the package (not a root-level script), so it's on your
PATH right after either install above — no separate clone or download step needed. It always
installs and wires up **market-data-hub** (the one supported way every function here reads
price/return data — see "Data source" below), asks only about the genuinely optional pieces
(dev/test tooling, Tree Studio's JS test harness — only offered in a repo checkout), and
locates or asks for the `.duckdb` database this environment should use, persisting it as
`MARKET_DATA_DB`. Idempotent — safe to re-run after upgrading or on a new machine. Pass
`--market-data-hub-path <dir>` to point at a specific checkout, or `--db-path <file.duckdb>` to
skip the interactive database prompt.
...
`setup_first_run.ps1` first (it builds the database and its ingestion pipeline); then run
`lazyportfolio-setup`, which will detect and reuse it.
...
unit tests), never because it's meant to be skipped in practice. `lazyportfolio-setup` always
...
```
src/lazyportfolio/_setup.py  guided install, callable as `lazyportfolio-setup`
src/lazyportfolio/
...
`lazyportfolio-setup` (see "Setup" above) is the recommended path. To install by hand instead:
